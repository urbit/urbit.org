...
Moons are the undeveloped wild outback of Urbit, with a story waiting to
be told that promises to be as enormous in scale as the cloud personal
...
resolves them, and speculate what the lunar locus of the Urbit ID
address space may become in the future. To keep the futurist scope
...
planet and a moon may be like one of a human and their smartphone. If you lose your
phone, you don't want someone to pretend they are you, operating
one of your moons, on the network. The planet can revoke the keys, and
...
clients, and who may only perform predetermined actions prescribed by those with the
servers. Running a server is theoretically accessible to anyone but, practically, is
gated by the fact that server maintenance is a professional occupation
rather than something simple like riding a bike. By making server maintenance comparable to bicycle maintenance, Urbit aspirationally levels the
playing field, elevating everyone to the rank of planet. Maybe not for the completely unskilled, but
within the capabilities of almost everyone willing to learn without an enormous
time investment, and never needed on a regular basis. We aren't quite
...
progress), one stopgap has been to download software by syncing a
...
again. For this reason, testing new software on the livenet is frequently performed on
...
philosophy, so any involvement of Urbit with IoT will soundly reject all of
...
internet connection is lost, the devices still work since they do not require one
to function. And if the manufacturer goes out of business,
...
By eliminating the SaaS aspect, Urbit solves many of the largest outstanding issues with
...
inevitably presented as SaaS that require intimate knowledge of your devices
and their stored data. Recommendation engines, such as the one used by YouTube, 
offer helpful guidance towards content you'll find appealing, but they're
also designed to keep you hooked to the service as long as possible. These kinds
of programs can be useful, but they're dystopian when they come
...
your digital personal AI assistant will live on one of your moons, 
rather than on megacorp's cloud.
...
Urbit is intended first and foremost as a personal
computer. When we unravel its potential usage in industry here, we
consider this in the context of a widely predicted future in which the trend
...
people. Thus, computation on modern automated farms may be thought of as existing at the
intersection of both personal and industrial scale. We predict that the set
of trades at this intersection will only grow over time.
...
Again, Urbit resolves these issues in the same fashion as above. One might argue that using Urbit is
...
SSIs communicating with an [Ames](@/docs/glossary/ames.md)-like networking
protocol and Urbit ID-style cryptographic identity, rather than Urbit itself.

For a SSI, losing power is less of a catastrophic event than it is for
...
always mean loss of any data potentially collected while power is down,
regardless of the device you use, but with a SSI you at least have
assurance that you will never lose data that was collected before
it could be transferred from the cache to the disk. Of course, this is
actually the sort of guarantee that [ACID
...
have a SSI. Put another way, every SSI is a database; having every device
be a database is a major boon in situations where enormous quantities of data
...
and be guaranteed that that moon will receive them and perform the tasks given, 
even if the device that originally sent the packets no longer exists. As
...
powerful guarantees on the integrity of those instructions no matter how much time
...
who it came from, so supplying such data puts one's reputation at stake.
...
due to the numerous problems with IoT. Imagine that you suffer
